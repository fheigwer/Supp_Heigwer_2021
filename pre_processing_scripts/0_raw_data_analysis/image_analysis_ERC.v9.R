...
analyze<-function(image_string){
  ...
  if(file.exists(nuc_image_name) && file.exists(body_image_name) && file.exists(mito_image_name) ){
    ...
    if(max(nuclei_objects)>30){    
      ...
      db_insert_into( con = test_db$con, table = "D1086_single_cell", values = cell_features)
      ...
      db_insert_into( con = test_db$con, table = "D1086_trimmed_mean", values = trim_mean_data)
      ...
      db_insert_into( con = test_db$con, table = "D1086_single_cell", values = mock)
      ...
      names(mock)=c("barcode","plate","screen","well","field",
                    ...
                    "fussel.sd",
                    "weak_tubulin.sd")
      db_insert_into( con = test_db$con, table = "D1086_trimmed_mean", values = mock)
