...
ui <- fluidPage(theme = "journal",
                fluidRow(
                  ...
                  #I will initially define 4 groups into which the cells will be classified:
                  ...
                  column(12, offset = 2, actionGroupButtons(inputIds = c("Normal", "Irregular_nucleus", "Big", "Condensed", "Skip"),
                                                            label =c("Normal", "Irregular nucleus", "Big", "Condensed", "Skip")))),
                  ...
                  column(8, offset = 1, radioButtons(inputId = "group",
                                                     label = "Cell state",
                                                     choices = list("Big", "Condensed", "Irregular_nucleus", "Normal"),
                                                     inline = TRUE)),
...
#######################################################################################################################################################################  
  ...
  observeEvent(c(input$Normal,input$Irregular_nucleus,input$Big, input$Condensed, input$Skip),{
  ...
  observeEvent(c(input$Normal,input$Irregular_nucleus,input$Big, input$Condensed, input$Unknown),{
    ...
    number_cells$table[3, "number_cells"] <- nrow(classified_cells$df %>% filter(group == "Big"))
    
    number_cells$table[4, "number_cells"] <- nrow(classified_cells$df %>% filter(group == "Condensed"))
    
    number_cells$table[5, "number_cells"] <- nrow(classified_cells$df)
    
    number_cells$table["group"] <- c("Normal", "Irregular_nucleus", "Big", "Condensed", "Total")
...
#DISPLAYED IMAGES
  ...
  observeEvent(c(input$Normal,input$Irregular_nucleus,input$Big, input$Condensed),{

    if(nrow(classified_cells$df)>10 & ((number_cells$table[5,"number_cells"]/50) %%1) == 0){
    ...
    if((number_cells$table[5,"number_cells"]/50) %%1 ==0){
  ...
  output$umap <- renderPlotly({
    ...
    if(nrow(classified_cells$df)>15){
      ...
      set.seed(100)
      umap_data <- classified_cells$df %>% select(numeric_features) %>% uwot::umap(n_neighbors = 30) %>% as.data.frame() %>% 
